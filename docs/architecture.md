...
| When does the adversarial pass get dispatched? | **As the FIRST action of a fresh turn, never the last of a build turn.** The builder builds, freezes, parks and notifies — then stops. CC-A dispatches the pass separately. | *Ruled 2026-09-01 after the SECOND stall at the same boundary.* Both stalls landed at the handoff into the black-box pass: once before the freeze, once after it and before dispatch. `dual-cc-session-design-v2.md` §5 names repeated stalls from one cause as a stop condition, and two is where it stops being noise. The diagnosis is structural, not a lapse: the dispatch had been the tail of a long build turn, at the moment the work feels finished and the next activity is qualitatively different. **The fix is not vigilance.** It also converts the stall into cadence — stopping after the freeze is now correct behaviour rather than a failure, which removes the ambiguity §8 complains about at the one boundary that kept producing it. **This does NOT touch the 2026-08-31 ownership ruling above: the builder still runs the pass as a black-box task, and independence remains a property of context isolation, not of who says "go."** Only the turn boundary moved. |
...
| 2026-09-01 | §5: the adversarial pass is dispatched as the first action of a fresh turn; the builder parks after freezing and stops. Ownership and independence unchanged. | Second stall at the same boundary, and `dual-cc-session-design-v2.md` §5 makes two the threshold. Recorded as a structural property of the handoff rather than as a builder failure — the previous shape asked for the hardest hand-off at the end of the longest turn. Also recorded because a reader meeting only the new rule would reasonably suspect the 2026-08-31 ownership amendment had been quietly reversed; it has not. |
